/*! revalidator-model 0.4.2 Original author Alan Plum <me@pluma.io>. Released into the Public Domain under the UNLICENSE. @preserve */
...
function model(schema) {
  ...
  function Model(data) {
    ...
    aug(self, deepCall(deepClone(Model.schema.defaults) || {}), filterObj(data || {}, function(v, k) {
  ...
  Model.prototype.dehydrate = function() {
    return transform(shallowClone(this), Model.schema.dehydrate);
  };
  Model.validate = function(data) {
    return revalidator.validate(data, Model.schema);
  };
  Model.hydrate = function(data) {
    var initial = deepCall(deepClone(Model.schema.defaults) || {});
